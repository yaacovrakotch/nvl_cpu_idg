...
from gadget.files import File, glob_one_only
...
def main():
    ...
    # Call Torch fixer
    # optimization: call only the BOM that matters (depending on PR). Call the routine from runner_botos.py later.
    swall = Elapsed()
    for tpproj in glob.glob(f'POR_TP/*/*.tpproj'):
        sw = Elapsed()
        cmd = f'{DoMV.get_torch_exe()} fix-tp -s {slnfile} -p {tpproj}'
