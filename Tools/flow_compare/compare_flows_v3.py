...
_MODULE_DIR = ROOT / "Modules" / "FUN" / "FUN_CCF_CX48"
...
def configure_module(module_dir,
                     flow_defs,
                     *,
                     mtpl_input=None,
                     out_suffix: str = "_bp",
    ...
    the per-group compare/symbolize work to the original generator without
    duplicating its logic. Mutates module globals so subsequent calls to
    `v3.main()` / `symbolize_mtpl.main()` operate on the new target.

    Args:
        module_dir: Module directory (e.g. .../Modules/FUN/FUN_CCF_CX48).
        flow_defs: OrderedDict of {flow_name: skill_subflow_prefix}.
        mtpl_input: Path to the .mtpl to read from. Default = `_orig.mtpl`.
        out_suffix: Output suffix for sidecar files (default `_bp`; auto-BP
            uses `_auto_bp_g{i}` per group).
        preseed_max: Cycle-aware symbol-numbering floor (default -1 = cycle 1).
        preseed_map: Cycle-aware reuse map (default empty).
    """
    ...
    _MODULE_NAME = _MODULE_DIR.name
    _BP_PATH = _MODULE_DIR / f"{_MODULE_NAME}{out_suffix}.mtpl"
    _ORIG_PATH = _MODULE_DIR / f"{_MODULE_NAME}_orig.mtpl"
    _SRC_MTPL = _MODULE_DIR / f"{_MODULE_NAME}.mtpl"
    ensure_orig_snapshot()
    MTPL_PRIMARY = Path(mtpl_input) if mtpl_input is not None else _ORIG_PATH
    MTPL_TEST_SOURCES = [MTPL_PRIMARY]
    OUT_CSV = _MODULE_DIR / f"{_MODULE_NAME}{out_suffix}.flows_compare.csv"
    PRESEED_MAX_SYMBOL = preseed_max
    PRESEED_SYMBOL_MAP = preseed_map or {}
    # Mark preseed as caller-supplied iff either argument is non-default.
    # `symbolize_mtpl.phase_compare` checks this flag to decide whether to
...
def derive_flow_defs(module_dir, flow_names) -> "OrderedDict[str, str]":
    """Auto-derive the {flow_name: skill_subflow_prefix} map by matching
    FlowItem instance names in the module's .mtpl against `Test Instance
    Name` rows in `SKILL_CSV`.

    For each flow F, the prefix is the namespace before `::` in skill rows
    whose post-`::` bare name appears as a FlowItem inside F's body in the
    .mtpl (after MTT-freq token reversion). The prefix with the highest
    intersection-count wins; ties broken by lexical order for determinism.

    Returns an OrderedDict matching the order of `flow_names`. If no prefix
    can be derived for a flow, raises RuntimeError (the original generator
    cannot proceed without a prefix).
    """
    md = Path(module_dir)
    name = md.name
    src = md / f"{name}_orig.mtpl"
    if not src.exists():
        src = md / f"{name}.mtpl"
    if not src.exists():
        raise FileNotFoundError(f"no mtpl in {md}")
    text = src.read_text(encoding="utf-8", errors="replace")
    nl = "\r\n" if "\r\n" in text[:4096] else "\n"
    lines = text.split(nl)

    # FlowItem instance set per flow (root + reachable sub-flows).
    flow_insts: dict[str, set[str]] = {}
    for fn in flow_names:
        s: set[str] = set()
        body_flows = [fn] + collect_subflows(lines, fn)
        for bf in body_flows:
            for it in parse_flow_body(lines, bf):
                s.add(it["instance"])
        flow_insts[fn] = s

    # Skill CSV: group bare-name set by prefix.
    if not SKILL_CSV.exists():
        raise FileNotFoundError(f"skill CSV missing: {SKILL_CSV}")
    prefix_bares: dict[str, set[str]] = {}
    with SKILL_CSV.open(encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            tname = row.get("Test Instance Name", "")
            if "::" not in tname:
                continue
            pfx, bare = tname.split("::", 1)
            pfx = pfx + "::"
            is_mtt = (row.get("Is MTT") or "").strip().lower() == "yes"
            if is_mtt:
                bare = _MTT_FREQ_REVERT.sub(r"_F\1_X_", bare)
            prefix_bares.setdefault(pfx, set()).add(bare)

    out: "OrderedDict[str, str]" = OrderedDict()
    for fn in flow_names:
        insts = flow_insts.get(fn, set())
        best_pfx = None
        best_n = 0
        for pfx, bares in prefix_bares.items():
            n = len(insts & bares)
            if n > best_n or (n == best_n and best_pfx is not None
                              and pfx < best_pfx):
                best_n = n
                best_pfx = pfx
        if best_pfx is None or best_n == 0:
            # No trace coverage for this flow. Emit a sentinel prefix that
            # cannot match any real `Test Instance Name` so trace-based
            # bucketing returns 0 rows. The hybrid `load_skill_csv()` then
            # fills this flow's instances entirely from the .mtpl Test
            # definitions.
            out[fn] = f"__NOTRACE_{fn}__::"
        else:
            out[fn] = best_pfx
    return out
...
# ---------------------------------------------------------------------------
# .mtpl Test-definition parsing (hybrid fallback for flows missing from
# the runtime trace `P16C_all.csv`).
# ---------------------------------------------------------------------------

_MTPL_TEST_DEF_RE = re.compile(
    r"^\s*(?P<kind>(?:CSharp)?(?:MultiTrial)?(?:Trial)?Test)\s+"
    r"(?P<rest>\S.*?)\s*$"
)


def _parse_mtpl_test_defs(lines: list[str]) -> dict[str, dict]:
    """Walk an .mtpl source and return a map
    `{instance_name: {'params': OrderedDict[name->value], 'is_mtt': bool}}`
    by parsing every top-level `Test`/`MultiTrialTest`/`CSharpTest`/
    `TrialTest`/`CSharpTrialTest` definition.

    Recognized grammar (one of):
      <Kind> <TemplateName> <InstanceName>
      <Kind> <InstanceName>
      <Kind> <TemplateName> "<InstanceName>"
    `<Kind>` is any identifier ending in `Test` (excluding `TestPlan`,
    `TestProgram`).

    The body is the next `{ ... }` block. Each non-blank line is treated
    as `Param = value;` (semicolon-terminated, possibly multi-line for
    long expressions — handled by depth tracking on `()[]{}` and quote
    state). Used by `load_skill_csv()` as a per-instance fallback when
    the runtime trace does not contain the instance.
    """
    out: dict[str, dict] = {}
    i = 0
    n = len(lines)
    while i < n:
        ln = lines[i]
        m = _MTPL_TEST_DEF_RE.match(ln)
        if not m:
            i += 1
            continue
        kind = m.group("kind")
        # Exclude TestPlan / TestProgram (file-level keywords).
        if kind in ("TestPlan", "TestProgram"):
            i += 1
            continue
        rest = m.group("rest").strip()
        # Parse the trailing tokens: either `<TemplateName> <InstanceName>`
        # or just `<InstanceName>`. Instance may be quoted.
        toks = rest.split()
        if not toks:
            i += 1
            continue
        last = toks[-1]
        # Strip quotes if instance is quoted.
        if len(last) >= 2 and last[0] == '"' and last[-1] == '"':
            inst = last[1:-1]
        else:
            inst = last
        if not inst or any(ch in inst for ch in "{};"):
            i += 1
            continue
        is_mtt = "MultiTrial" in kind

        # Find the opening brace and parse the block.
        j = i
        ...
        body_lines = lines[j + 1:end]
        params = _parse_mtpl_block_params(body_lines)
        out[inst] = {"params": params, "is_mtt": is_mtt}
        i = end + 1
    return out


def _parse_mtpl_block_params(body_lines: list[str]) -> "OrderedDict[str, str]":
    """Parse the body of a Test definition into `{param_name: value}`.
    Joins logical statements (terminated by `;` outside quotes/brackets),
    then splits each into name/value pairs. Strips outer quotes from values
    using the same rules as `parse_params()` for trace data."""
    result: "OrderedDict[str, str]" = OrderedDict()
    blob = "\n".join(body_lines)
    # Walk char-by-char, splitting on top-level `;`.
    depth = 0
    in_q = False
    buf: list[str] = []
    statements: list[str] = []
    ...
    for stmt in statements:
        ...
        name = name.strip()
        val = val.strip()
        # Collapse internal whitespace runs that came from line breaks.
        val = re.sub(r"\s+", " ", val)
        if not name:
            continue
...
# Set to True by `configure_module()` when an orchestrator (e.g.
# `auto_run_groups.py`) explicitly supplies a preseed map. When True,
# `symbolize_mtpl.phase_compare` MUST honor the caller-set values and
...
FLOW_DEFS = OrderedDict([
    ...
    ("FUN_CCF_CX48_F2XCCF", "F2XCCF_SubFlow_SPEEDPRL2CPU::"),
    ("FUN_CCF_CX48_F3XCCF", "F3XCCF_SubFlow_SPEEDPRL2CPU::"),
])
FLOWS = list(FLOW_DEFS.keys())
...
def load_skill_csv() -> dict[str, list[dict]]:
    """Hybrid per-flow test-data loader. Trace-first, then fills per-flow
    gaps from `.mtpl` Test definitions. Trace data wins when both sources
    cover the same (flow, instance) pair, since trace carries
    runtime-resolved values (e.g. real frequency tokens).

    The returned structure is unchanged: `{flow_name: [{instance, params,
    is_mtt}, ...]}` — so all downstream code (`v3.main`, symbolize_mtpl)
    needs no changes.
    """
    out: dict[str, list[dict]] = {fn: [] for fn in FLOWS}
    seen_per_flow: dict[str, set[str]] = {fn: set() for fn in FLOWS}

    # Phase A: bucket trace rows by FLOW_DEFS prefix (original behavior).
    if SKILL_CSV.exists():
        with SKILL_CSV.open(encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                ...
                for fn, prefix in FLOW_DEFS.items():
                    if tname.startswith(prefix):
                        ...
                        # runtime: skill returns the resolved freq token
                        # (e.g. `_F1_1200_`) but the source mtpl declares
                        # them with the templated `_F1_X_` form. Revert it.
                        if is_mtt:
                            bare = _MTT_FREQ_REVERT.sub(r"_F\1_X_", bare)
                        out[fn].append({
                            "instance": bare,
                            "params": parse_params(
                                row["Test Parameters"] or ""),
                            "is_mtt": is_mtt,
                        })
                        seen_per_flow[fn].add(bare)
                        break

    # Phase B: hybrid fallback. For each flow, walk every reachable
    # FlowItem (root + sub-flows). For instances NOT covered by trace,
    # synthesize a skill row from the .mtpl Test definition.
    if not MTPL_PRIMARY.exists():
        return out
    ...
    mtpl_defs = _parse_mtpl_test_defs(primary_lines)

    added = 0
    missing_def = 0
    for fn in FLOWS:
        body_flows = [fn] + collect_subflows(primary_lines, fn)
        seen_inst: set[str] = set()
        for bf in body_flows:
            for it in parse_flow_body(primary_lines, bf):
                inst = it["instance"]
                if inst in seen_per_flow[fn] or inst in seen_inst:
                    continue
                seen_inst.add(inst)
                td = mtpl_defs.get(inst)
                if td is None:
                    missing_def += 1
                    continue
                out[fn].append({
                ...
                seen_per_flow[fn].add(inst)
                added += 1
    if added or missing_def:
        print(f"  hybrid-fill: +{added} mtpl-derived instance(s); "
              f"{missing_def} FlowItem(s) had no Test def in mtpl")
    return out
...
def main() -> int:
    ...
    # For ordinal fallback: build per-flow ordered instance lists
    per_flow_instances = {fn: [it for _, it in per_flow_keys[fn]] for fn in FLOWS}
    ...
    # Top-level entity row: the flow names themselves
    flow_lines = []
    for fn in FLOWS:
    ...
    for idx, key in enumerate(ordered_keys):
        ...
        # params: align by name, then fallback to ordinal if missing
        param_names = []
        seen_p = set()
        for it in items:
            if not it:
