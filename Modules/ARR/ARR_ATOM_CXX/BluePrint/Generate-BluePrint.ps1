<#
.SYNOPSIS
    Transforms ARR_ATOM_CXX.mtpl into a BluePrint (BP) file + corresponding prompt.

.DESCRIPTION
    IDG-aligned blueprint generator for the CXX cross-socket module.
    A blueprint is "a minimized, symbolized version of the full module MTPL file"
...
.EXAMPLE
    .\Generate-BluePrint.ps1 -InputMtpl "..\ARR_ATOM_CXX.mtpl"
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)]
...
# --- ECC_FREQ (F5/F6/F7 for ECCON/ECCSBFDV in F5-domain tests) -----------
# MUST come before FREQ_CORNER to avoid F5/F6/F7 being eaten by generic patterns
$allReplacements += New-Symbol -Name 'ECC_FREQ' `
    -Description 'ECC frequency corner for ECCON/ECCSBFDV tests within F5-domain flows. F5=lowest, F7=highest.' `
    -Values @('F5','F6','F7') `
    -Pairs @(
        # ECCON_VMINTC test names: _VCCIA_F5_X_ etc. (only for F5XAT/F5XATLO flow, not main F1-F4)
        @{Find='_VCCIA_F5_X_L2DATA_ECCON_VMINTC'; Replace='_VCCIA_\ECC_FREQ\_X_L2DATA_ECCON_VMINTC'},
        @{Find='_VCCIA_F6_X_L2DATA_ECCON_VMINTC'; Replace='_VCCIA_\ECC_FREQ\_X_L2DATA_ECCON_VMINTC'},
        @{Find='_VCCIA_F7_X_L2DATA_ECCON_VMINTC'; Replace='_VCCIA_\ECC_FREQ\_X_L2DATA_ECCON_VMINTC'},
        # ScreenTC ECCON names
        @{Find='SCREEN_ECCON_VMIN_F5'; Replace='SCREEN_ECCON_VMIN_\ECC_FREQ\'},
        @{Find='SCREEN_ECCON_VMIN_F6'; Replace='SCREEN_ECCON_VMIN_\ECC_FREQ\'},
        @{Find='SCREEN_ECCON_VMIN_F7'; Replace='SCREEN_ECCON_VMIN_\ECC_FREQ\'},
        # CtvTokensTC ECCSBFDV test names within F5XAT/F5XATLO (F5/F6/F7 in _VCCIA_Fn_X_ECCSBFDV)
        @{Find='_VCCIA_F5_X_ECCSBFDV'; Replace='_VCCIA_\ECC_FREQ\_X_ECCSBFDV'},
        @{Find='_VCCIA_F6_X_ECCSBFDV'; Replace='_VCCIA_\ECC_FREQ\_X_ECCSBFDV'},
        @{Find='_VCCIA_F7_X_ECCSBFDV'; Replace='_VCCIA_\ECC_FREQ\_X_ECCSBFDV'},
        # RunCallback ECCON json params
        @{Find='per_array_f5.json'; Replace='per_array_\ecc_freq\.json'},
        @{Find='per_array_f6.json'; Replace='per_array_\ecc_freq\.json'},
        @{Find='per_array_f7.json'; Replace='per_array_\ecc_freq\.json'},
        # SetPointsPreInstance AT_F5/F6/F7_FREQ (only in ECC tests, distinct from main F1-F4)
        @{Find='AT_F5_FREQ'; Replace='AT_\ECC_FREQ\_FREQ'},
        @{Find='AT_F6_FREQ'; Replace='AT_\ECC_FREQ\_FREQ'},
        @{Find='AT_F7_FREQ'; Replace='AT_\ECC_FREQ\_FREQ'},
        # VminResult ECCON keys
        @{Find='F5_VMIN_ECCON'; Replace='\ECC_FREQ\_VMIN_ECCON'},
        @{Find='F6_VMIN_ECCON'; Replace='\ECC_FREQ\_VMIN_ECCON'},
        @{Find='F7_VMIN_ECCON'; Replace='\ECC_FREQ\_VMIN_ECCON'},
        # VMAXXATLO ECCSBFDV tests
        @{Find='_VCCIA_F5_X_ECCSBFDV'; Replace='_VCCIA_\ECC_FREQ\_X_ECCSBFDV'},
        @{Find='_VCCIA_F6_X_ECCSBFDV'; Replace='_VCCIA_\ECC_FREQ\_X_ECCSBFDV'},
        @{Find='_VCCIA_F7_X_ECCSBFDV'; Replace='_VCCIA_\ECC_FREQ\_X_ECCSBFDV'},
        # RunCallback ECCON test name F5/F6/F7 component
        @{Find='_VCCIA_F5_X_L2DATA_ECCON_VMINTC'; Replace='_VCCIA_\ECC_FREQ\_X_L2DATA_ECCON_VMINTC'},
        @{Find='_VCCIA_F6_X_L2DATA_ECCON_VMINTC'; Replace='_VCCIA_\ECC_FREQ\_X_L2DATA_ECCON_VMINTC'},
        @{Find='_VCCIA_F7_X_L2DATA_ECCON_VMINTC'; Replace='_VCCIA_\ECC_FREQ\_X_L2DATA_ECCON_VMINTC'}
    )
...
$allReplacements += New-Symbol -Name 'FREQ_CORNER' `
    ...
    -Pairs @(
        ...
        # ECCON test name freq component for F1-F4 (ECCON_VMIN_F1 etc.)
        @{Find='ECCON_VMIN_F1'; Replace='ECCON_VMIN_\FREQ_CORNER\'},
        @{Find='ECCON_VMIN_F2'; Replace='ECCON_VMIN_\FREQ_CORNER\'},
        @{Find='ECCON_VMIN_F3'; Replace='ECCON_VMIN_\FREQ_CORNER\'},
        @{Find='ECCON_VMIN_F4'; Replace='ECCON_VMIN_\FREQ_CORNER\'},
        # ScreenTestSet ECCON per-freq
        @{Find='ECCON_VMIN_F1_PMUX_OR_NOT'; Replace='ECCON_VMIN_\FREQ_CORNER\_PMUX_OR_NOT'},
        @{Find='ECCON_VMIN_F2_PMUX_OR_NOT'; Replace='ECCON_VMIN_\FREQ_CORNER\_PMUX_OR_NOT'},
        @{Find='ECCON_VMIN_F3_PMUX_OR_NOT'; Replace='ECCON_VMIN_\FREQ_CORNER\_PMUX_OR_NOT'},
        @{Find='ECCON_VMIN_F4_PMUX_OR_NOT'; Replace='ECCON_VMIN_\FREQ_CORNER\_PMUX_OR_NOT'},
        # VminResult ECCON_L2DATA keys for F1-F4
        @{Find='F1_VMIN_ECCON_L2DATA'; Replace='\FREQ_CORNER\_VMIN_ECCON_L2DATA'},
        @{Find='F2_VMIN_ECCON_L2DATA'; Replace='\FREQ_CORNER\_VMIN_ECCON_L2DATA'},
        @{Find='F3_VMIN_ECCON_L2DATA'; Replace='\FREQ_CORNER\_VMIN_ECCON_L2DATA'},
        @{Find='F4_VMIN_ECCON_L2DATA'; Replace='\FREQ_CORNER\_VMIN_ECCON_L2DATA'},
        # CtvTokensTC PrePlist VMIN_ECCON_L2DATA refs for F1-F4
        @{Find='F1_VMIN_ECCON'; Replace='\FREQ_CORNER\_VMIN_ECCON'},
        @{Find='F2_VMIN_ECCON'; Replace='\FREQ_CORNER\_VMIN_ECCON'},
        @{Find='F3_VMIN_ECCON'; Replace='\FREQ_CORNER\_VMIN_ECCON'},
        @{Find='F4_VMIN_ECCON'; Replace='\FREQ_CORNER\_VMIN_ECCON'},
        # StartVoltages forwarded from Fn results
        @{Find='D.RF1AT'; Replace='D.R\FREQ_CORNER\AT'},
        @{Find='D.RF2AT'; Replace='D.R\FREQ_CORNER\AT'},
        @{Find='D.RF3AT'; Replace='D.R\FREQ_CORNER\AT'},
        @{Find='D.RF4AT'; Replace='D.R\FREQ_CORNER\AT'}
    )

# --- COVERAGE_TYPE (COMBINED / SSA_L2DATA) --------------------------------
$allReplacements += New-Symbol -Name 'COVERAGE_TYPE' `
    -Description 'Coverage variant for array tests: COMBINED=all arrays, SSA_L2DATA=L2 data arrays.' `
...
# --- ECC_MODULE (M0-M3 for CtvTokensTC ECCSBFDV tests) -------------------
$allReplacements += New-Symbol -Name 'ECC_MODULE' `
    -Description 'ECCSBFDV CtvTokensTC module index. M0=base (no suffix), M1-M3 have _M1/_M2/_M3 suffixes.' `
    -Values @('M0 (no suffix)','M1','M2','M3') `
    -Pairs @(
        # Test name suffix for M1/M2/M3 (M0 has no suffix)
        @{Find='_L2_DATA_M1'; Replace='_L2_DATA_\ECC_MODULE\'},
        @{Find='_L2_DATA_M2'; Replace='_L2_DATA_\ECC_MODULE\'},
        @{Find='_L2_DATA_M3'; Replace='_L2_DATA_\ECC_MODULE\'},
        # PrePlist VoltageConverter overrides
        @{Find='--overrides ATOM0:ATOMM0'; Replace='--overrides ATOM\ECC_MODULE_NUM\:ATOMM\ECC_MODULE_NUM\'},
        @{Find='--overrides ATOM1:ATOMM1'; Replace='--overrides ATOM\ECC_MODULE_NUM\:ATOMM\ECC_MODULE_NUM\'},
        @{Find='--overrides ATOM2:ATOMM2'; Replace='--overrides ATOM\ECC_MODULE_NUM\:ATOMM\ECC_MODULE_NUM\'},
        @{Find='--overrides ATOM3:ATOMM3'; Replace='--overrides ATOM\ECC_MODULE_NUM\:ATOMM\ECC_MODULE_NUM\'}
    )
...
while ($i -lt $rawLines.Count) {
    ...
    elseif ($line -match '^\s*Flow\s+') {
        # Stop at Flow section — don't process flow items, only test definitions
        break
    }
    else {
        $i++
...
function Normalize-ForDedup {
    ...
    # Strip BypassPort variations (TPRules with different args)
    $n = $n -replace 'BypassPort\s*=\s*ARR_ATOM_CXX_Rules\.TPRules\([^)]+\)', 'BypassPort = \BYPASS_PORT_RULES\'
    return $n
}
...
[void]$pr.AppendLine("- CXX is a cross-socket module with no _CPU1 dual-die variants.")
[void]$pr.AppendLine("- F5 corner uses ATOM_TOP trial variable and Corners.AT_C5/FreqValues.AT_C5.")
[void]$pr.AppendLine("- F4XATLO/F5XATLO are low operating point variants of F4XAT/F5XAT.")
[void]$pr.AppendLine("- ECCON tests use ECC-on patterns with ECCSBFDV CtvTokensTC for fail data viewer.")
[void]$pr.AppendLine("- RETRY tests repeat ECCON with different ScreenTestSet (e.g. F5_TICK_UP).")
[void]$pr.AppendLine("- FMIN corner uses different timing TC and voltage forwarding from F1 results.")
[void]$pr.AppendLine("- LTTC tests use different patlist (lttc), LimitGuardband, StartVoltages (D.RFnAT).")
[void]$pr.AppendLine("- ECCON_FLOAT is a standalone FMIN ECCON test with BypassPort=1.")
