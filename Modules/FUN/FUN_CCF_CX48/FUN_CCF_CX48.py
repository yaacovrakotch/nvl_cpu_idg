...
def get_test_apextc(flow, corner, core_corner, FlowMatrix, subflow, content_list):
   ...
   for test_type, test_parms in content_list.items():
      test_list_apextcflow.append(ApexTC(name=f'SBFT_CCF_VMIN_K_{flow}_X_X_{corner}_X_{test_type}_APEXTC',
         ...
         TimingsTc="FUN_CCF_CX48::cpu_fun_timing_mts800_tstprtclk400_tck100_fun_ccf_n2p",
         ...
         VoltageConverter=  Spec(f'"--overrides CCF:"+__shared__::FlowMatrixSingular.CCF_VMAX_VALUE+", CORE1:"+FUN_CCF_CX48_SPECS.CR_VMAX_VALUE+",CORE0:"+FUN_CCF_CX48_SPECS.CR_VMAX_VALUE+" --railconfigurations CDIE_CCF_POWERMUX CDIE_CCF_NBLCTRL --dlvrpins VCCIA --expressions CCF_CORE"'),
         ExportTokens = "FXCCF",
    ...
    for test_type, test_parms in content_list.items():
        ...
        test_list_funflow.append(NativeMultiTrial(
            ...
            template=VminTC(
                ...
                DtsConfiguration = Spec(f'FUN_CCF_CX48_SPECS.dts_default'),
                LevelsTc="CPU_IP_BASE::cpu_all_bf_x_x_ipc_lvl_min",
                TimingsTc="IPC::CPU_IP_BASE::cpu_hvm_timing_mts200_tstprtclk100_tck25",
                #TimingsTc="FUN_CCF_CX48::cpu_fun_timing_mts400_tstprtclk200_tck50_i_drv_mul_fmin_fun_CCF",
                ...
                FivrCondition="NOM_CCF_CORE",
                FivrConditionPlistParamName="Patlist",
                VoltageConverter=  Spec(f'"--overrides CORE1:"+FUN_CCF_CX48_SPECS.CR_VMAX_VALUE+",CORE0:"+FUN_CCF_CX48_SPECS.CR_VMAX_VALUE+" --railconfigurations CDIE_CCF_POWERMUX CDIE_CCF_NBLCTRL --dlvrpins VCCIA --expressions CCF_CORE"'),
                StepSize=Spec(f"toDouble(__shared__::FlowMatrixSingular.CR_SEARCH_RESOLUTION)"),
                PreInstance = Spec(f'FUN_CCF_{sku}_SPECS.UncoreConfigPreinstance'),
                SetPointsPreInstance=Spec(f'PSPRE.CLR_{corner}+","+"MCdrv:"+' +Spec(f'FUN_CCF_{sku}_SPECS.corefreq+":\"') +Spec(f"+ __shared__::FlowMatrixSingular.CR_{core_corner}+") +'"GHz,MCdrv:"+' +Spec(f'FUN_CCF_{sku}_SPECS.ringfreq+":\"') +Spec(f"+ __shared__::FlowMatrixSingular.{FlowMatrix}+") +'"GHz,CORE:nblctrl_core_l2:nbloff,MCdrv:R1_SET,"+FUN_CCF_CX48_SPECS.MCfunCCF_FMIN_DRVMASK'),
                SetPointsPostInstance=Spec(f'PSPOST.CLR_{corner}+","+"CORE:nblctrl_core_l2:nblon,"+FUN_CCF_CX48_SPECS.MCfunCCF_FMIN_DRVUNMASK'),
            ),
...
def get_test_list_fminfailflow(flow, corner, core_corner, FlowMatrix, subflow, content_list):
    ...
    for test_type, test_parms in content_list.items():
        ...
        test_list_funflow.append(VminTC(name=f"SBFT_CCF_VMIN_K_{subflow}_X_CCF_{corner}_X_{test_type}_FAILFLOW",
                ...
                PreInstance=Spec(f'FUN_CCF_{sku}_SPECS.UncoreConfigPreinstance'),
                SetPointsPreInstance=Spec(f'PSPRE.CLR_{corner}+","+"MCdrv:"+' +Spec(f'FUN_CCF_{sku}_SPECS.corefreq+":\"') +Spec(f"+ __shared__::FlowMatrixSingular.CR_{core_corner}+") +'"GHz,MCdrv:"+' +Spec(f'FUN_CCF_{sku}_SPECS.ringfreq+":\"') +Spec(f"+ __shared__::FlowMatrixSingular.{FlowMatrix}+") +'"GHz,CORE:nblctrl_core_l2:nbloff,MCdrv:R1_SET,"+FUN_CCF_CX48_SPECS.MCfunCCF_FMIN_DRVMASK'),
                SetPointsPostInstance=Spec(f'PSPOST.CLR_{corner}+","+"CORE:nblctrl_core_l2:nblon,"+FUN_CCF_CX48_SPECS.MCfunCCF_FMIN_DRVMASK'),
                _fitem=Fitem('SAME', edc=test_parms["IS_EDC"],
        ...
        if (
            ...
            test_listt_ScreenGSDS_3.append(
                ScreenTC(
                    ...
                    ScreenTestSet=test_parms.get("ScreenTestSet", "FUN_CCF_MM_SCREEN_CX48_SingleFlow"),
    ...
    for test_type, test_parms in content_list.items():
        if re.match(r"^DRG", test_type):
            # Generate a variant for each corner
                ...
                test_list_shmoo.append(
                    DDGShmooTC(
                        ...
                        TimingsTc=TimingsTc,
                        VoltageConverter=Spec(f'"--overrides CORE1:"+FUN_CCF_CX48_SPECS.CR_VMAX_VALUE+",CORE0:"+FUN_CCF_CX48_SPECS.CR_VMAX_VALUE+" --railconfigurations CDIE_CCF_POWERMUX CDIE_CCF_NBLCTRL --dlvrpins VCCIA --expressions CCF_CORE --fivrcondition NOM_CCF_CORE"'),
            ...
            if test_type == "DRNG":
                ...
                VoltageConverter=Spec(f'"--overrides CORE3:"+FUN_CCF_CX48_SPECS.CR_VMAX_VALUE+",CORE2:"+FUN_CCF_CX48_SPECS.CR_VMAX_VALUE+",CORE1:"+FUN_CCF_CX48_SPECS.CR_VMAX_VALUE+",CORE0:"+FUN_CCF_CX48_SPECS.CR_VMAX_VALUE+" --railconfigurations CDIE_CCF_POWERMUX CDIE_CCF_NBLCTRL --dlvrpins VCCIA --expressions CCF_CORE --fivrcondition NOM_CCF_CORE"')
                SetPointsPreInstance=Spec(f'"MCdrv:" + FUN_CCF_{sku}_SPECS.corefreq + ":0.8GHz,MCdrv:" + FUN_CCF_{sku}_SPECS.ringfreq + ":0.8GHz,CORE:nblctrl_core_l2:nblon,CCF:nblctrl_ccf:ccfnblon"')
                LevelsTc = "CPU_IP_BASE::cpu_all_bf_x_x_ipc_lvl_nom"
                TimingsTc="CPU_IP_BASE::cpu_fun_timing_mts100_tstprtclk50_tck100"
            else:
...
for sku in product_skus:                                                    # <<<<<<<< This one
    ...
    if sku == "CX48":
       ...
       "SetPoint": "FUN_CCF_CORESTATE0_MASK"}
        ...
        INIT_FUSE_RPT_STICKY_ERROR = {
       "RPT_STICKY_ERROR": {"Bypass": 1,"IS_EDC": False, "Plist": "fun_cdie_vccr_sbft_f1xccf_fc_drg_hptp800_list_master",
       "ConfigurationFile": Spec(MODULEPATH + ' + ' + f'"./Modules/FUN/FUN_CCF_{sku}/InputFiles/FUN_CCF_Generic.PatConfigSetpoints.json"'),
       "SetPoint": "R8_RPT_STICKY_ERROR"}
        }
    ...
    INIT_FUSE_RPT_STICKY_ERROR = get_init_fuse_list(INIT_SUBFLOW, INIT_CORNER, INIT_FUSE_RPT_STICKY_ERROR)
    ...
    INIT_FUSE_COMP = Flow("FUSE_PATMOD", INIT_FUSE_FC_DRG,INIT_FUSE_CCFBIST,INIT_FUSE_CORESTATE0_MASK,INIT_FUSE_RPT_STICKY_ERROR)
    INIT_PATMOD_COMP = Flow("PATMOD_WAITTIME", INIT_content_WAITTIME_DRG)
    INIT_TCK_COMP = Flow("FUN_CCF_CX48_INIT_TCK", INIT_TCK_Tests)
    ...
    if sku == "CX48":
       ...
       "FC_DRG"    : {"Bypass" : -1, "IS_EDC" : False} # Define the test types we want in this subflow and also define Bypass and EDC definitions separately so that we can change them easily in the future.
        ...
        XCCFF5_SCREENTC = {
       "SINGLEFLOW"     :{"Bypassport": -1, "IS_EDC" : True, "ScreenTestSet":"FUN_CCF_MM_SCREEN_CX48_SingleFlow"}
       }
    else:
        {
        }
...
# Example for XCCFF5_Subflow (apply this pattern to all similar usages)
    ...
    ImpactStudyLO_Tests = get_impactstudy_list(XCCFF5_Flow, XCCFF5_Corner, XCRF5_Corner, XCCFF5_FlowMatrix, XCCFF5_SubFlow, ImpactStudyLO_content_list)

    #################################################################
    #         
    #                     FMAXLO AND F5LO SUBFLOW                      
    #
    #################################################################
    XCCFF5_Subflow = Flow(
   ...
   # Input
    if sku == "CX48":
       ...
       "FC_DRG"    : {"Bypass" : Spec(f"__shared__::TpRule.If_DS0_DS1_M(-1, 1, -1, 1)"), "IS_EDC" : False} # Define the test types we want in this subflow and also define Bypass and EDC definitions separately so that we can change them easily in the future.
       }
       ...
       "FC_DRG"    : {"Bypass" : -1, "IS_EDC" : True} # Define the test types we want in this subflow and also define Bypass and EDC definitions separately so that we can change them easily in the future.
        ...
        XCCFFMIN_SCREENTC = {
       "SINGLEFLOW"     :{"Bypassport": -1, "ScreenTestSet":"FUN_CCF_MM_SCREEN_CX48_SingleFlow"}
       }
    ...
    SHMOO_COMP = Flow("FUN_CCF_CX48_SHMOO", *SHMOO_Tests)
...
# Example for XCCFF4_Subflow (apply this pattern to all similar usages)
    ...
    END_SUBFLOW = Flow('FUN_CCF_CX48_ENDCPU @END_SubFlow', END_VMINTC_UUFC_TEST,
