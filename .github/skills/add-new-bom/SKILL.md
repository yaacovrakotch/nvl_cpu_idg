...
#### D1d. Check for UTF-8 Byte Order Mark (BOM) encoding in copied files

> ⚠️ **Critical — A UTF-8 Byte Order Mark (BOM) in copied files causes pymtpl compiler failure and broken batch files.**
>
> **Note:** In this context "Byte Order Mark" is an invisible file encoding prefix — it is NOT
> related to the Bill of Materials (BOM) / bomgroup concept used throughout the rest of this skill.
>
> When files are copied using Windows tools (VS Code, Notepad, etc.) that default to
> "UTF-8 with BOM", the copied files may contain an invisible 3-byte prefix (`EF BB BF`).
> The pymtpl compiler will throw:
> ```
> Unknown stpl line: [\ufeffVersion 1.0;]
> ```
> Batch files with a Byte Order Mark will fail with:
> ```
> 'ï»¿@echo' is not recognized as an internal or external command
> ```

After copying, run this PowerShell snippet to detect and strip any Byte Order Marks from `.stpl` and `.bat` files:

```powershell
$dir = "POR_TP\<NEW_POR_TP_FOLDER>"
$files = Get-ChildItem -Path $dir -Recurse -File |
    Where-Object { $_.Extension -match '\.(stpl|bat)$' } |
    Where-Object {
...
foreach ($f in $files) {
    ...
    Write-Host "Byte Order Mark stripped: $($f.FullName)"
}
```

If the command reports any stripped files, those files were corrupted and must be committed
without the Byte Order Mark. The most critical file is `SubTestPlan.stpl` — a Byte Order Mark
here will prevent all ProgramFlows compilation for this Bill of Materials (bomgroup).
...
File: `NVL_<DIELET>.sln` (e.g., `NVL_HUB.sln`, `NVL_CPU.sln`, etc.)

The solution file requires **two additions per new project** — both are mandatory. A project
block alone is not enough; without a `NestedProjects` entry the project appears at the
solution root instead of inside the correct folder.

#### D5a. Add the Project block for the new `.tpproj`

Insert a new `Project(...)` ... `EndProject` block immediately after the source BOM's
equivalent block. Copy the block from the source BOM and update the name, path, and GUID.
Generate a new GUID:
```powershell
[System.Guid]::NewGuid().ToString("B").ToUpper()
```
...
Example:
```
Project("{B37224B3-71CF-4949-84BE-8D0AA0EF5C98}") = "Class_NVL_<SHORT_BOM>", "POR_TP\Class_NVL_<SHORT_BOM>\Class_NVL_<SHORT_BOM>.tpproj", "{<NEW-GUID>}"
EndProject
```

#### D5b. Add the `NestedProjects` entry for the `.tpproj` (under `POR_TP`)

In `GlobalSection(NestedProjects)`, add an entry to place the new project under the
`POR_TP` solution folder. Find the `POR_TP` folder GUID by looking at the existing
nesting entries for the other `Class_NVL_*` `.tpproj` projects (they all share the same
parent GUID).

```
{<NEW-GUID>} = {<POR_TP-FOLDER-GUID>}
```

> ⚠️ **Common mistake (seen in S28CB bringup):** Forgetting this line causes the project
> to appear at the solution root instead of inside the `POR_TP` folder. Always verify by
> searching `NestedProjects` for the new GUID after editing.

#### D5c. Add entries for dielet-specific BaseInputs projects (if applicable)

Some dielet repos include per-BOM projects under `BaseInputs/<dielet>/`
(e.g., `HUB_Class_NVL_<SHORT_BOM>` as an `.ipctproj` in `nvl.hub`). If the source BOM
has such a project registered in the solution, do the same for the new BOM:

1. Add a `Project(...)` ... `EndProject` block pointing to the new
   `BaseInputs/<dielet>/<NEW_FOLDER>/<NEW_FOLDER>.ipctproj` (or equivalent extension).
2. In `NestedProjects`, add `{<IPCTPROJ-GUID>} = {<BASEINPUTS-HUB-FOLDER-GUID>}` to
   nest it under the correct `BaseInputs/<dielet>/` solution folder.

> The parent folder GUID can be found by looking at the existing nesting entries for
> the other `HUB_Class_NVL_*` (or equivalent) sibling projects.
...
- [ ] `POR_TP/<NEW_POR_TP_FOLDER>/` exists with all internal references updated
- [ ] No UTF-8 Byte Order Mark in `SubTestPlan.stpl` or any `.bat` files (run Step D1d check)
- [ ] `pf_pymtpl.bat` references the new BOM
- [ ] Checker workflow yml exists and references the new BOM name correctly
- [ ] Solution file includes the new `.tpproj` `Project(...)` block
- [ ] Solution file `NestedProjects` has entry nesting the `.tpproj` under the `POR_TP` folder
- [ ] Solution file `NestedProjects` has entry nesting any dielet BaseInputs project (e.g., `HUB_Class_NVL_*`) under its correct folder
