...
from qgates.cttrbnmdie_chk import *        # replace this with your checker py.
...
class TestInstName(TestCase):
    ...
    '''
            ...
            print("Running test_error_203...")
            obj = CttrPupComp(tpobj)
            obj.main()
            # pprint(obj.result)
            expect = [{'id': 203,
                       'message': 'Base#:22071 in CDIE CHK '
                                  'test:CA2TF_ATOM_VMAX_K_CATF6_X_VATOM_F5_3800_FUSA_VMAX_RECOVERY_1501 '
                                  'should start with number:1',
                       'module': 'ARR'}]
            self.assertEqual(obj.result, expect)
            self.assertEqual(len(obj.passed), 0)

    def test_error_204(self):
        with TempDir(name=True, chdir=True, delete=True) as tdir:
            tpref = f'{UT_DIR_REPO}/Simple7f'
            shutil.copytree(tpref, f'{tdir}/TPL')
            envfile = f'TPL/POR_TP/TGLH81/EnvironmentFile.env'
            ...
            Test VminTC CA2TF_ATOM_VMAX_K_CGTF6_X_VATOM_F5_3800_FUSA_VMAX_RECOVERY_1501
            {
                BypassPort = -1;
                BaseNumbers = "12071";
                TestMode = "SingleVmin";
            ...
            DUTFlow MAIN1  {
                DUTFlowItem CA2TF_ATOM_VMAX_K_CGTF6_X_VATOM_F5_3800_FUSA_VMAX_RECOVERY_1501 CA2TF_ATOM_VMAX_K_CGTF6_X_VATOM_F5_3800_FUSA_VMAX_RECOVERY_1501
                {
                    Result -2
                    {
                        Property PassFail = "Fail";
...
'''
            File('TPL/Modules/ARR/array.mtpl').touch(arr_mtpl, mkdir=True, newfile=True)
            tpobj = TestProgram(envfile).init()
            print("Running test_fail_case_200_201...")
            obj = CttrPupComp(tpobj)
            obj.main()
            # pprint(obj.result)
            expect = [{'id': 204,
                      'message': 'Base#:12071 in GDIE CHK '
                                 'test:CA2TF_ATOM_VMAX_K_CGTF6_X_VATOM_F5_3800_FUSA_VMAX_RECOVERY_1501 '
                                 'should start with number:2',
                       'module': 'ARR'}]
            self.assertEqual(obj.result, expect)
            self.assertEqual(len(obj.passed), 0)

    def test_error_205(self):
        with TempDir(name=True, chdir=True, delete=True) as tdir:
            tpref = f'{UT_DIR_REPO}/Simple7f'
            shutil.copytree(tpref, f'{tdir}/TPL')
            envfile = f'TPL/POR_TP/TGLH81/EnvironmentFile.env'
            ...
            Test VminTC CA2TF_ATOM_VMAX_K_CSNF3_X_VATOM_F5_3800_FUSA_VMAX_RECOVERY_1501
            {
                BypassPort = -1;
                BaseNumbers = "22071";
                TestMode = "SingleVmin";
            ...
            DUTFlow MAIN1  {
                DUTFlowItem CA2TF_ATOM_VMAX_K_CSNF3_X_VATOM_F5_3800_FUSA_VMAX_RECOVERY_1501 CA2TF_ATOM_VMAX_K_CSNF3_X_VATOM_F5_3800_FUSA_VMAX_RECOVERY_1501
                {
                    Result -2
                    {
                        Property PassFail = "Fail";
...
'''
            ...
            # pprint(obj.result)
            expect = [{'id': 205,
                      'message': 'Base#:22071 in SOC CHK '
                                 'test:CA2TF_ATOM_VMAX_K_CSNF3_X_VATOM_F5_3800_FUSA_VMAX_RECOVERY_1501 '
                                 'should start with number:4',
                       'module': 'ARR'}]
            self.assertEqual(obj.result, expect)
            self.assertEqual(len(obj.passed), 0)
