...
```
STARTPRL0HUB_SubFlow      STARTHUBNOM_SubFlow        rm2fm2 rm1fm1 r0f40 r40f40
```
...
### Rule 3 — `extra_unmanaged` in `ProgramFlowsHUB.py` (or other dielet-pkg files) requires user confirmation

Unmanaged entries in `ProgramFlowsHUB.py` do not have a predictable PRL-inner-subflow pattern. Always confirm with the user whether the entry is intentional before dismissing it.
...
| `ProgramFlowsHUB.py` / `ProgramFlowsSharedPKG.py` | Any unrecognized entry | ⚠️ Confirm with user |
